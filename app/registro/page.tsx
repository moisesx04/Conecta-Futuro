      ...
      <main className="min-h-screen bg-white flex items-center justify-center p-6 text-center font-sans">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md">
          <div className="w-24 h-24 bg-blue-50 rounded-[32px] flex items-center justify-center mx-auto mb-8 shadow-xl shadow-blue-100 relative">
            ...
            <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute -top-2 -right-2 w-8 h-8 bg-red-600 rounded-full flex items-center justify-center">
          ...
          <p className="text-slate-500 font-bold mb-10 leading-relaxed uppercase text-xs tracking-widest">
            Tu solicitud ha sido enviada. ¡Estamos listos para conectar tu futuro!
          </p>
          <button onClick={() => window.location.reload()} className="w-full py-5 bg-blue-600 hover:bg-blue-700 text-white rounded-[24px] font-black shadow-2xl shadow-blue-600/20 transition-all active:scale-95">
            Finalizar
          </button>
        </motion.div>
      </main>
          ...
          {step === 0 && (
            <motion.div key="step0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
              <div>
                ...
                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">Presiona ENTER para continuar</p>
              </div>
              <input 
                autoFocus
                type="text" 
